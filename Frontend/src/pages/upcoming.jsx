...
function Upcoming() {
    ...
    if (error) {
        return (
            <div className="min-h-[60vh] bg-zinc-950 text-white flex flex-col items-center justify-center px-4 relative overflow-hidden">
                <div className="absolute -left-10 -top-10 h-72 w-72 rounded-full bg-amber-800/10 blur-[80px] pointer-events-none"></div>
                <div className="absolute -right-10 -bottom-10 h-72 w-72 rounded-full bg-yellow-800/10 blur-[80px] pointer-events-none"></div>
                <div className="relative z-10 flex flex-col items-center max-w-md w-full text-center px-6 py-8 rounded-3xl border border-amber-500/15 bg-zinc-900/40 backdrop-blur-xl shadow-2xl">
                    <div className="relative w-16 h-16 mb-6 flex items-center justify-center rounded-2xl bg-amber-950/50 border border-amber-500/30 text-amber-500">
                    ...
                    <button onClick={fetchUpmovies} className="px-6 py-3 rounded-xl bg-linear-to-r from-amber-600 to-yellow-500 hover:from-amber-500 hover:to-yellow-400 text-white font-extrabold uppercase text-xs tracking-wider transition-all duration-300 shadow-[0_4px_15px_rgba(245,158,11,0.2)] hover:scale-[1.02] active:scale-[0.98] cursor-pointer">
                        Retry
                    </button>
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen bg-zinc-950 px-4 py-12 sm:px-8">
            {/* Heading */}
            <div className="relative mb-10 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between border-l-4 border-amber-500 pl-4">
                <div>
                ...
                <div className="flex w-fit items-center gap-1.5 rounded-full border border-amber-500/20 bg-amber-950/20 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-400 backdrop-blur-sm animate-pulse">
            ...
            {loading ? (
                ...
                <div className="grid gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                    {upmovies.map((movie) => (
                        <div
                            key={movie.id}
                            className="group relative cursor-pointer overflow-hidden rounded-2xl border border-white/5 bg-white/1 p-2 backdrop-blur-md transition-all duration-300 hover:border-amber-500/30 hover:bg-white/4 hover:shadow-[0_12px_40px_rgba(245,158,11,0.2)]"
                        >
                            <div className="relative overflow-hidden rounded-xl aspect-2/3 bg-zinc-900">
                                ...
                                <div className="absolute inset-0 bg-linear-to-t from-zinc-950 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                                    <span className="text-[10px] font-bold text-amber-400 bg-amber-950/60 border border-amber-500/30 px-2 py-0.5 rounded-full backdrop-blur-sm shadow-sm">
                                        Expected: High 📈
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-3">
                                ...
                                <button
                                    ...
                                    className="mt-3 w-full rounded-xl bg-white/5 hover:bg-amber-600 border border-white/10 hover:border-amber-600 py-2 text-xs font-bold text-zinc-300 hover:text-white transition-all duration-300 shadow-sm cursor-pointer"
                                >
                                    Know More
                                </button>
                            </div>
